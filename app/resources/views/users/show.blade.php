        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow-lg h-100">
                ...
                <div class="card-body text-center">
                    ...
                    {{-- ユーザーがログインユーザー自身の場合のみ、ボタンを表示 --}}
                    @if (Auth::check() && Auth::id() === $user->id)
                        <div class="mb-3">
                            {{-- ★ 依頼・リクエスト一覧へのリンク (維持) ★ --}}
                            <a href="{{ route('requests.index') }}" class="btn btn-primary btn-block shadow mb-2">
                                <i class="fas fa-handshake mr-2"></i>依頼・リクエスト一覧
                            </a>
                            <a href="{{ route('profile.edit') }}" class="btn btn-outline-primary btn-block shadow">
        ...
        <div class="col-md-8">
            ...
            <div class="card shadow-lg mb-4">
                <div class="card-header bg-danger text-white h5">
                    <i class="fas fa-heart mr-2"></i>{{ $user->name }} がいいねした投稿
                </div>
                ...
                <div class="card-body p-0">
                    ...
                    @else
                        <ul class="list-group list-group-flush">
                            @foreach ($likedServices as $service)
                                <li class="list-group-item d-flex justify-content-between align-items-center p-3">

                                    <div class="d-flex align-items-center">

                                        {{-- サムネイル --}}
                                        <img src="{{ $service->image ? asset('storage/' . $service->image) : 'https://placehold.co/50x50/ef4444/ffffff?text=Img' }}"
                                            style="width: 50px; height:50px; object-fit:cover;"
                                            class="rounded mr-3">
                                        <div>
                                            <a href="{{ route('services.show', $service) }}"
                                                class="h6 mb-0 text-dark font-weight-bold">
                                                {{ $service->title }}
                                            </a>
                                            <p class="text-success small mb-0 font-weight-bold">
